    ...
    for (size_t i = 0; i < input_files.size(); ++i) {
       ...
       // Don't make a copy on Posix. Slash is a path separator, back slashes are not accepted as a substitute.
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if (load_type != LoadType::LoadGeometry) {
                            if (en_3mf_file_type == En3mfType::From_BBS)
                                show_info(q, _L("The 3mf is generated by old Orca-Flashforge, load geometry data only."), _L("Load 3mf"));
                            else
                                show_info(q, _L("The 3mf is not supported by Orca-Flashforge, load geometry data only."), _L("Load 3mf"));
                        }
        ...
        for (ModelObject *model_object : model.objects) {
            ...
            wxString msg = wxString::Format(_("Loading file: %s"), from_path(real_filename));
            model_idx++;
            dlg_cont = dlg.Update(progress_percent, msg);
            if (!dlg_cont) {
                q->skip_thumbnail_invalid = false;
